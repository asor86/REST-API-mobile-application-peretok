...
class getMagazines
{
    ...
    public function __construct($count, $offset)
    {
        $this->count = $count;
        $this->offset = $offset;
    }
    ...
    public function init()
    {
        ...
        $DB_ELEMENT = CIBlockElement::GetList(
            Array("SORT" => "DESC"),
            Array("IBLOCK_ID" => IB_MAGAZINES, "ACTIVE" => "Y"),
            false,
            array("iNumPage" => $this->offset, "nPageSize" => $this->count,),
            Array("ID", "IBLOCK_ID", "NAME", "PREVIEW_TEXT", "DATE_ACTIVE_FROM", "PREVIEW_PICTURE", "PROPERTY_pdf")
        );
        $arElements = array();
        while ($ob = $DB_ELEMENT->GetNext()) {
            ...
            $tmp['id'] = $ob['ID'];
            $tmp['title'] = $ob['NAME'];
            $tmp['discr'] = self::decode_entities_full(strip_tags($ob['PREVIEW_TEXT']));
...
//                $tmp['preview'] = 'http://' . $_SERVER['SERVER_NAME'] . CFile::GetPath($ob['PREVIEW_PICTURE']);
            ...
            $tmp['pdf_link'] = 'http://' . $_SERVER['SERVER_NAME'] . CFile::GetPath($ob['PROPERTY_PDF_VALUE']);
            $arElements[] = $tmp;
        }
    ...
    public static function decode_entities_full($string, $quotes = ENT_COMPAT, $charset = 'UTF-8')
    {

        return html_entity_decode(preg_replace_callback('/&([a-zA-Z][a-zA-Z0-9]+);/', function ($matches, $destroy = true) {
